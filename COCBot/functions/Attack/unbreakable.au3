...
Func Unbreakable()
	...
	While _CheckPixel($aSurrenderButton, True) = False
		If _Sleep($iDelayUnbreakable1) Then Return True; wait for clouds to disappear and the end battle button to appear
		If $i > 45 Then
	...
	While _CheckPixel($aSurrenderButton, True) = True
		...
		If _Sleep($iDelayUnbreakable1) Then Return True; wait for button to disappear
	...
	If CheckObstacles() = True Then  ; Check for unusual windows open, or slow windows
		If _Sleep($iDelayUnbreakable3) Then Return True; wait for window to close
